<?php 
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST")
    {
        ...
        {
            // FIX #8: inicializar $resultado antes de usarlo
            $resultado = false;
            ...
            if(isset($_POST["username_a_seguir"]))
            {
                ...
                if($receptor_id && isset($_SESSION["id"]))
                {
                    ...
                    if ($resultado) {
                        checkMisionesSeguir($conexion, $_SESSION["id"]);
                        checkMisionesSeguidores($conexion, $receptor_id);
                    }
                }
            }
    ...
    if(empty($usuario_perfil)) { header("Location: /views/app.php"); exit; }

    $usuario_sesion = $_SESSION["username"];
    $conexion = conexion();

    $datosUsuario = obtenerDatosUsuario($conexion, $usuario_perfil);
    if(!$datosUsuario) { header("Location: /views/app.php"); exit; }
